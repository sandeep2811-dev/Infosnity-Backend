...
// 1. Trust Proxy (Required for Render/Vercel to handle cookies correctly)
...
app.use(
    session({
        secret: process.env.SESSION_SECRET || "infty_secret_789", 
        resave: false,
...
========================= */
app.get("/", (req, res) => {
    res.send("Infty Backend is Live and Running!");
});
...
io.on("connection", (socket) => {
    ...
    socket.on("send_class_group_message", async (data) => {
        try {
            const { classId, sender, senderName, message, file_url, file_type } = data;
...
// Use process.env.PORT for Render, fallback to 5000 for local
